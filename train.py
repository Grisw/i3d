        ...
        for phase in ['train', 'val']:
            ...
            for data["rgb"], data["flow"], labels in data_loaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    for stream in streams:
                        ...
                        running_f1s[stream].append(f1_score(labels.data.cpu().numpy(), preds.numpy(), average='macro'))
            ...
            for stream in losses.keys():
                ...
                log('{} Loss ({}): {:.4f} Acc: {:.4f} F1: {:.4f}'.format(
                    phase, stream, epoch_losses[stream], epoch_accs[stream], epoch_f1s[stream]))
            ...
            for stream in streams:
                ...
                if phase == 'val' and epoch_f1s[stream] > best_f1s[stream]:
                    ...
                    best_model_wts[stream] = copy.deepcopy(models[stream].state_dict())
            if phase == 'val' and epoch_accs['composed'] > best_accs['composed']:
                best_accs['composed'] = epoch_accs['composed']
            if phase == 'val' and epoch_f1s['composed'] > best_f1s['composed']:
                best_f1s['composed'] = epoch_f1s['composed']
        print()
...
if __name__ == "__main__":
    ...
    data_dir = Path('data/images/pre-processed')
    streams = ["rgb", "flow"]
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    data_paths = []
    for class_name in class_names:
        for video in os.listdir(data_dir / 'all' / class_name):
            data_paths.append((class_name, video, data_dir / 'all' / class_name / video))
    print(f'load {len(data_paths)} samples.')
    np.random.shuffle(data_paths)



    num_each_val = int(len(data_paths) / args.cv)
    for cv in range(args.cv):
        log(f'---------start cv {cv}---------------')
        dirs = {'train': [], 'val': []}
        for i, data_path in enumerate(data_paths):
            if cv * num_each_val <= i < (cv + 1) * num_each_val:
                dirs['val'].append(data_path)
            else:
                dirs['train'].append(data_path)

        args.session_id = f'cv{cv}'

        rgb_flow_datasets = {x: RGBFlowDataset(dirs[x], class_dicts,
